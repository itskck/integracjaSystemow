...
class ComponentsCubit extends Cubit<ComponentsState> {
  ...
  Future<void> saveToDatabase() async {
    ...
    for (var row in rows) {
      final List<String> laptopData = row.split(';');
      final LaptopsCompanion lapComp = LaptopsCompanion.insert(
        manufacturer: laptopData[0],
        screenSize: laptopData[1],
        resolution: laptopData[2],
        screenType: laptopData[3],
        touchScreen: laptopData[4],
        cpuName: laptopData[5],
        cpuCores: int.tryParse(laptopData[6]) ?? 0,
        cpuSpeed: int.tryParse(laptopData[7]) ?? 0,
        ram: laptopData[8],
        discCapacity: laptopData[9],
        discType: laptopData[10],
        gpuName: laptopData[11],
        gpuVram: laptopData[12],
        osName: laptopData[13],
        opticalDriveName: laptopData[14],
      );
      db.saveLaptop(lapComp);
    }
  }
